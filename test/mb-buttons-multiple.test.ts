...
describe('MbButtons-Multiple', () => {
  ...
  it('emits data on input', async () => {
    const mbbuttons = await fixture<MbButtonsMultiple>(
      html`<mb-buttons-multiple label="Hello there">
    ...
    expect(event2.target.data).to.eql([{code: 'option1', value: 'Option 1', terminology: 'local'},{code: 'option2', value: 'Option 2', terminology: 'local'}]);

    buttons[0].click()
    const event3: any = await oneEvent(mbbuttons, 'mb-input');
    expect(event3.target.data).to.eql([{code: 'option2', value: 'Option 2', terminology: 'local'}]);
  });

  it('emits data with correct terminology', async ()=>{
    const mbbuttons = await fixture<MbButtonsMultiple>(
      html`<mb-buttons-multiple label="Hello there" terminology="SOME-TERMINOLOGY">
      <mb-option value="option1" label="Option 1"></mb-option>
      <mb-option value="option2" label="Option 2"></mb-option>
    </mb-buttons-multiple>`
    );
    ...
    buttons[1].click()
    const event2: any = await oneEvent(mbbuttons, 'mb-input');
    expect(event2.target.data).to.eql([{code: 'option1', value: 'Option 1', terminology: 'SOME-TERMINOLOGY'},{code: 'option2', value: 'Option 2', terminology: 'SOME-TERMINOLOGY'}]);
  })
  it('changes input on setting data', async () => {
    ...
    mbbuttons.data = [{code: 'option1', value: 'Option 1', terminology: 'SOME-TERMINOLOGY'},{code: 'option2', value: 'Option 2', terminology: 'SOME-TERMINOLOGY'}]
    await oneEvent(mbbuttons, 'mb-input')
    await elementUpdated(mbbuttons);

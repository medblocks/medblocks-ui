...
describe('MbButtons', () => {
  ...
  it('changes input on setting data', async () => {
    ...
    expect(buttons[0]).to.not.class('button--primary')
    expect(buttons[1]).to.class('button--primary')
    expect(buttons[2]).to.not.class('button--primary')

    mbbuttons.data = {code: 'option3', value: 'Option 3', terminology: 'SOME-TERMINOLOGY'}
    await oneEvent(mbbuttons, 'mb-input')
    await elementUpdated(mbbuttons);
    expect(buttons[0]).to.not.class('button--primary')
    expect(buttons[1]).to.not.class('button--primary')
    expect(buttons[2]).to.class('button--primary')
  });
});
